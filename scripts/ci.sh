...
run_e2e_tests() {
    ...
    case "$test_type" in
        "sanity")
            ...
            E2E_MOCK_MODE=true pnpm exec playwright test tests/e2e/functional/sanity.spec.ts --workers=2
            ;;
        "smoke")
            log "📋 Step $((step_number + 1)): E2E Smoke Tests (smoke)"
            E2E_MOCK_MODE=true pnpm exec playwright test tests/e2e/functional/smoke.spec.ts --workers=2
            ;;
        "critical")
            log "📋 Step $((step_number + 1)): E2E Critical Tests (critical)"
            E2E_MOCK_MODE=true pnpm exec playwright test tests/e2e/functional/critical.spec.ts --workers=2
            ;;
        "responsive")
            log "📋 Step $((step_number + 1)): E2E Responsive Tests (responsive)"
            E2E_MOCK_MODE=true pnpm exec playwright test tests/e2e/functional/responsive.spec.ts --workers=2
            ;;
        "cross-browser")
            log "📋 Step $((step_number + 1)): E2E Cross-Browser Tests (cross-browser)"
            E2E_MOCK_MODE=true pnpm exec playwright test tests/e2e/functional/cross-browser.spec.ts --workers=2
            ;;
        "full")
