...
// Conventional Commits regex pattern with ticket number and username
...
function validateCommitMessage(message: string): ValidationResult {
  ...
  // Check if subject follows conventional commits format with ticket and username
  ...
  const [, type, scope, breaking, ticketNumber, username, description] = match;
  ...
  if (breaking && !message.includes('BREAKING CHANGE:')) {
    result.warnings.push('Breaking change indicator (!) used but no BREAKING CHANGE: footer found');
  }
  ...
  const subjectWords = subject.split(':')[1]?.trim().split(' ')[0]?.toLowerCase();
  if (subjectWords && !imperativeWords.some(word => subjectWords.startsWith(word))) {
    result.warnings.push(
      'Consider using imperative mood in the subject line (e.g., "add" not "added")'
    );
  }
