#!/usr/bin/env tsx
/**
 * @fileoverview Optimized environment-aware database reset script
 * This script consolidates environment loading and reduces redundant operations
 */
...
// Cache for migration file timestamps
const migrationTimestamps = new Map<string, number>();

async function resetWithEnvironmentOptimized(options: ResetOptions) {
  const {
    env,
  ...
  // Check if database is already clean (skip if requested)
  if (skipIfClean) {
    console.log(`🔍 Checking if database is already clean...`);
    try {
      const checker = new SchemaConsistencyChecker();
      const consistencyResult = await checker.check();

      if (consistencyResult.success) {
        console.log(`✅ Database is already clean, skipping reset`);
        return { success: true, skipped: true };
      }
    } catch (error) {
      console.log(`⚠️  Could not check database state, proceeding with reset`);
    }
  }
  ...
  // Check migration file timestamps to avoid unnecessary copies
  if (checkTimestamps) {
    console.log(`🔍 Checking migration file timestamps...`);
    const migrationsDir = 'src/lib/db/migrations';
    const customMigrationsDir = 'drizzle';

    if (existsSync(migrationsDir) && existsSync(customMigrationsDir)) {
      const migrationFiles = readdirSync(migrationsDir).filter(f => f.endsWith('.sql'));
      let needsCopy = false;

      for (const file of migrationFiles) {
        const sourcePath = join(migrationsDir, file);
        const destPath = join(customMigrationsDir, file);
        ...
        const sourceTime = statSync(sourcePath).mtime.getTime();
        const destTime = statSync(destPath).mtime.getTime();
      ...
      if (!needsCopy) {
        console.log(`✅ Migration files are up to date, skipping copy`);
      } else {
        console.log(`📋 Migration files need updating, copying...`);
        if (!dryRun) {
  ...
  if (dryRun) {
    console.log(`🔍 DRY RUN: Would execute database reset operations`);
...
async function main() {
  ...
  try {
    const result = await resetWithEnvironmentOptimized(options);
...
export { resetWithEnvironmentOptimized };
