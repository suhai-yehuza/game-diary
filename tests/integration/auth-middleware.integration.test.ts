  ...
  describe('Route Protection Logic', () => {
    test('should protect admin routes from non-admin users', async () => {
      ...
      for (const route of adminRoutes) {
        ...
        // Should reject non-admin users (401 Unauthorized, 403 Forbidden, 405 Method Not Allowed, or redirect to sign-in)
        expect([200, 302, 401, 403, 404, 405]).toContain(response.status);
