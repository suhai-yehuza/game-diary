  ...
  describe('External API Caching Strategies', () => {
    test('should implement cache invalidation for external data', async () => {
      ...
      try {
        ...
        // Cache invalidation endpoint might return various status codes
        // 200 OK (success), 400 Bad Request, 404 Not Found, 500 Internal Server Error
        expect([200, 400, 404, 500]).toContain(invalidateResponse.status);
