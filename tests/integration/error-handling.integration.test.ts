...
describe('Error Handling Integration Tests', () => {
  describe('Centralized Error Handling', () => {
    ...
    test('should handle API errors with proper status codes', async () => {
      const errorScenarios = [
        { endpoint: '/api/user/invalid-id', expectedStatus: [401, 404, 400, 500] },
        { endpoint: '/api/search?q=', expectedStatus: [400, 200, 500] },
        { endpoint: '/api/admin/database/nonexistent', expectedStatus: [404, 401, 403, 500] },
  ...
  describe('Error Logging and Monitoring', () => {
    ...
    test('should handle error severity levels', async () => {
      ...
      for (const endpoint of severityEndpoints) {
        try {
          const response = await fetchWithTimeout(`${BASE_URL}${endpoint}`, {}, 5000); // Reduced timeout
          ...
          if (response.status >= 400) {
            try {
              ...
              // If JSON parsing fails, that's acceptable for error responses
          ...
          if (
            ...
            console.warn(`Request to ${endpoint} timed out or was aborted, skipping...`);
            continue;
          }
          throw error;
        }
        ...
        if (response.status >= 500) {
          ...
          if (response.headers.get('content-length') !== '0' && response.body) {
            try {
              ...
              // The important thing is that we got a proper error status
