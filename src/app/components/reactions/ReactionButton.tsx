...
export function ReactionButton({
  ...
  return (
    <button
      onClick={handleClick}
      className={cn(
        'inline-flex items-center gap-1.5 rounded-full border transition-all duration-150 hover:scale-105 active:scale-95',
        sizeClasses[size],
        hasReacted
    ...
    >
      ...
      {showCount && count > 0 && <span className="font-medium">{count}</span>}
    </button>
  );
}
