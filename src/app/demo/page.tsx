'use client';

import Link from 'next/link';
import React from 'react';

export default function DemosIndexPage() {
  const demos = [
    {
      title: 'Comments',
      description: 'View comments with their reactions and child comments',
      href: '/demo/individual-comments',
      features: [
        'Protected and public comment viewing',
        'Reaction grouping by emoji',
        'Paginated replies loading',
        'Comment counts',
        'Performance metrics display',
      ],
      status: 'Available',
    },
    // Add more demos here as they're created
  ];

  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Demo Components</h1>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Interactive demos showcasing optimized GraphQL queries and React hooks for the Game
            Diary application. These components demonstrate best practices and provide examples for
            integration.
          </p>
        </div>

        {/* Demos Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {demos.map(demo => (
            <div key={demo.href} className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-start justify-between mb-4">
                <h2 className="text-xl font-semibold text-gray-900">{demo.title}</h2>
                <span className="px-2 py-1 bg-green-100 text-green-800 text-xs font-medium rounded-full">
                  {demo.status}
                </span>
              </div>

              <p className="text-gray-600 mb-4">{demo.description}</p>

              <div className="mb-4">
                <h3 className="text-sm font-medium text-gray-900 mb-2">Features:</h3>
                <ul className="space-y-1">
                  {demo.features.map(feature => (
                    <li key={feature} className="flex items-center text-sm text-gray-600">
                      <span className="w-1.5 h-1.5 bg-blue-500 rounded-full mr-2" />
                      {feature}
                    </li>
                  ))}
                </ul>
              </div>

              <Link
                href={demo.href}
                className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
              >
                View Demo
                <svg className="ml-2 w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                  />
                </svg>
              </Link>
            </div>
          ))}
        </div>

        {/* Usage Instructions */}
        <div className="mt-12 bg-white rounded-lg shadow-sm p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">How to Use These Demos</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-2">For Developers</h3>
              <ul className="space-y-2 text-sm text-gray-600">
                <li>• Test new GraphQL queries and hooks</li>
                <li>• Validate data structures and performance</li>
                <li>• Copy components as starting points</li>
                <li>• Understand implementation patterns</li>
              </ul>
            </div>
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-2">For Integration</h3>
              <ul className="space-y-2 text-sm text-gray-600">
                <li>• See real-time data flow and loading states</li>
                <li>• Test error handling and edge cases</li>
                <li>• Verify UI/UX patterns and styling</li>
                <li>• Monitor performance metrics</li>
              </ul>
            </div>
          </div>
        </div>

        {/* Development Note */}
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
          <div className="flex">
            <div className="flex-shrink-0">
              <svg className="h-5 w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
                <path
                  fillRule="evenodd"
                  d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
            <div className="ml-3">
              <h3 className="text-sm font-medium text-yellow-800">Development Environment</h3>
              <p className="mt-1 text-sm text-yellow-700">
                These demo components are only available in development mode. They provide
                interactive examples of the GraphQL queries and React hooks for testing and
                integration purposes.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
