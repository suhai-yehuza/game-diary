...
export default function GameLogDetailPage({ params: _params }: IGameLogDetailPageProps) {
  ...
  const gameLogId = routeParams?.gameLogId as string;
  ...
  const queryVariables = useMemo(() => {
    ...
    return undefined;
  }, [gameLogId, isSignedIn, isLoaded, user]);
  ...
  if (!isSignedIn && !(process.env.NODE_ENV === 'development' && gameLogId)) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20">
        <div className="container mx-auto px-4 py-8">
          <div className="mb-6">
  ...
  if (error || !gameLog) {
    ...
    const stillLoading =
      process.env.NODE_ENV === 'development' ? !gameLogId : !gameLogId || !isSignedIn || !isLoaded;
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-neutral-600 dark:text-neutral-400 mt-1">
                  Game Log by @{gameLog.user?.first_name || gameLog.user?.username || 'Anonymous'}
                </p>
              </div>
